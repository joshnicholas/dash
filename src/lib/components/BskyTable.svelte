<script>
	...
	function shouldExclude(item) {
		// Check if Topic is in the exclude array
		if (exclude.includes(item.Topic)) {
			return true;
		}
		return false;
	}
	...
	let filteredData = $derived(fuzzySearch(excludedData, searchTerm, 0.5, ['Topic']));
...
<div class="w-full" style="margin-bottom: 30px;">
	...
	{#if filteredData.length > 0}
		<p class="text-xs">
			Last scraped: ~{formatMelbourneDateTime(data[0]?.scraped_datetime)}
		</p>
	{/if}
			...
			<div class="max-h-96 overflow-y-scroll">
				<table class="w-full bg-transparent table-fixed">
					<thead class="sticky top-0" style="background-color: #ffd861;">
						<tr>
							<th class="px-1 py-2 text-left text-xs font-bold tracking-wider">Topic</th>
						...
						{#each filteredData as item, index}
							<tr class="{index % 2 === 1 ? 'bg-yellow-100' : ''}" style="{index % 2 === 1 ? 'background-color: #fce18d;' : ''}">
								<td class="px-1 py-2 text-sm">
									{item.Topic} -
									<a href="https://bsky.app{item.Url}" target="_blank" rel="noopener noreferrer"
										   class="font-bold">
											Link
									</a>
								</td>
